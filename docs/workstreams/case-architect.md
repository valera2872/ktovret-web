# Mystery Logic — Case Architect Workstream
...
Build a repeatable Case Factory for strong Premium investigations, using:
- Mystery Logic authoring rules;
- Leslie Grant-Adamson as mandatory detective-writing reference;
- current competitor benchmark;
- existing Mystery Logic cases/analytics;
- adversarial QA;
- simulated playtests;
- existing Release Gate + Cognitive Gate.

Near-term business purpose:
grow the Premium catalogue before aggressive conversion optimization / Club promotion.

## Priority decision

On 2026-09-21 the user set Case Architect as the current priority.
...
3. Private per-case artifact template.
4. Independent internal audit roles.
5. Simulated playtest protocol.
6. Explicit NOT READY stop conditions.
7. Competitor originality check before CANON.
8. No private canon in public GitHub.

## Quality target

Premium is not "more text".

Each case should aim for:
- fast hook;
- real investigative action;
- multiple viable hypotheses;
- causal red herrings;
- varied evidence;
- multi-step deduction;
- credible motives and lies;
- fair play;
- reconstruction;
- post-case explanation;
- memorable format-specific experience.

## Portfolio target

Existing monetization strategy uses approximately 6–8 strong Premium investigations as the working threshold before aggressive Club promotion.

The Case Architect should therefore optimize the portfolio, not produce random isolated cases.

## Current portfolio questions

Before creating the next case, inventory current sellable/near-sellable Premium titles by:
- Solo / Partner / Party / AI / Live;
- duration;
- difficulty;
- setting;
- incident type;
...
Then identify the most valuable missing slot.

## First execution

CASE-ARCH-001 should NOT immediately write a full mystery.

First run:
1. inventory current Premium catalogue;
2. identify 3–5 portfolio gaps;
3. benchmark current competitor catalogue;
4. propose 2–4 concept directions for the highest-value gap;
5. run originality/product-fit gate;
6. present only survivors for user selection.

After concept selection:
CANON -> characters -> evidence -> contradictions -> player plot -> audits -> playtests -> release gates.

## Safety / privacy

Case private canon is stored only in private Project/Library/server-side materials.

Public workstream may record:
- lifecycle stage;
- case ID;
- non-spoiler role/format;
- branch/PR/implementation status;
- gate status;
- exact next step.

Do not record culprit/solution/private evidence in public GitHub.

## Checkpoint schema for each case
...
- Lifecycle stage
- Approved decisions
- Rejected concepts/approaches
- Private canon location
- Evidence graph status
- Suspect matrix status
- Contradiction map status
- Benchmark/originality status
- Simulated playtest status
- Human blind test status
- G0–G9
- Cognitive Gate
