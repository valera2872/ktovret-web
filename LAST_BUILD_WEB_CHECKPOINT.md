...
Следующий шаг: опубликовать `0.2.10` в текущую feature-ветку, дождаться `validate` и `deploy-preview`, затем повторить в live preview короткий маршрут по четырём исправленным точкам. Только после подтверждения — high-reuse art assets: утренний офис и 4 портрета. AI interrogation остаётся B-upgrade после устойчивого базового web-flow.
