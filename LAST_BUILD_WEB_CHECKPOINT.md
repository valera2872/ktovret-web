...
- CI автоматически проверяет, что client и Edge Function fallback используют один и тот же словарь тем.
...
### Конкурентный gate после redesign `0.4.0`

Повторная live-ревизия первого рабочего экрана показала, что проблема была не в количестве механик, а в одновременной конкуренции интерфейсных зон. После cold open игрок видел большой заголовок, три счётчика, четыре раздела, длинную вводную, пустой блок действий, шесть материалов и три боковых статуса. Это заставляло сначала разбираться в интерфейсе, а не в деле.

Сравнительный вывод:
- Last Ascent и ProfileDetective сначала дают один сильный материальный образ дела;
- Saint Twins сначала ориентирует через briefing / newspaper / address book, а уже затем отдаёт свободу маршрута;
- Dramtezi показывает одну сцену и только после неё подключает инструменты;
- для Mystery Logic нужен тот же эффект ориентации, но без линейного сценария и без потери самостоятельного выбора.

**A — реализовано обязательно:**
- счётчики прогресса и правая статусная колонка удалены с рабочего стола;
- четыре основных раздела собраны в одну компактную горизонтальную навигацию;
- рабочий стол использует документальную сцену офиса как единый визуальный якорь;
- на свежем старте показаны ровно три первичных материала: сообщение Павла, утренний осмотр и первые показания;
- полная вводная свёрнута и не конкурирует с первым действием;
- пустой блок действий больше не показывается; проверки появляются только когда действительно earned;
- на обзорном экране одновременно показывается не больше трёх ближайших материалов;
- в архиве первичные материалы отделены от остальных поступлений.

**Что нельзя потерять:** cold open, одна объективная истина, независимые маршруты, authored interrogation, premium evidence renderers, ручной proof-board и персональный debrief. Redesign не изменяет канон или доказательный граф.

**B — рассматривать после нового ручного прохождения:** отдельные документальные миниатюры только для трёх стартовых материалов, если одних различимых типов носителей окажется недостаточно. Не создавать изображения до подтверждённого UX-эффекта.

**C — не делать:** видео-заставка, 3D-рабочий стол, большая карта, анимация каждого документа. Они эффектны, но не решают задачу ориентации дешевле уже реализованной прогрессивной подачи.

QA `0.4.0`: `Validate advanced investigations` SUCCESS и `Deploy Last Build lab preview` SUCCESS на commit `86cceb1a78b66e2a5f29635d4203c7fb4dad30b7`; live desktop review подтверждает новую визуальную иерархию. Mobile/desktop Chrome smoke проверяет три стартовых материала и отсутствие dashboard counters / status sidebar.
