...
Статус: draft; web `0.5.2`, APK/Flutter заморожен.
...
Текущий статус `0.5.2`: **validate SUCCESS + deploy-preview SUCCESS + live browser review SUCCESS**. Проверены последовательные первичные опросы, plain-language роли и термины, понятность завязки без IT-знаний, кинематографическая обложка, progressive opening, strict interrogation pilot, четыре gated-состояния и authored eval 45/45.
...
### Конкурентный gate после cinematic opening `0.5.0`

Live-ревизия `0.4.0` показала следующий уровень проблемы: интерфейс стал чище, но аккуратный рабочий стол всё ещё не создавал интригу или ощущение большого продукта. Три стартовых материала оставались тремя равноправными решениями, а визуальный якорь не рассказывал историю сам по себе.

**A — реализовано обязательно:**
- создана новая документальная обложка: телефон Павла и пустой футляр ORBIT-2 в утреннем офисе;
- обложка существует постоянно, поэтому её видят и новый игрок, и игрок с сохранённым прогрессом;
- отдельный блокирующий overlay cold open заменён встроенным сюжетным входом без лишнего стартового клика;
- на первом экране одна доминирующая реплика: «Один из вас уже продал нашу игру»;
- три коротких факта сразу объясняют состояние дела без чтения длинной вводной;
- одна крупная кнопка ведёт к следующему осмысленному материалу: сообщение → осмотр → показания;
- свобода не потеряна: весь архив по-прежнему доступен через «Материалы»;
- после перехода в рабочие разделы обложка автоматически становится компактной;
- первичные действия и остальные карточки не конкурируют с обложкой до завершения базовой ориентации.

Новый WebP имеет размер около 46 КБ и даёт эффект premium hero без видео, анимации или отдельной сцены запуска.

**B — только после ручной реакции игроков:** минимальная микродинамика света/фокуса или короткий звуковой акцент. Реализовывать лишь при доказанном приросте вовлечения.

**C — не делать:** видео-трейлер внутри дела, 3D-пролёт по офису, полноэкранная анимированная заставка. Они создадут задержку и стоимость, но не сделают следующий шаг понятнее.

QA `0.5.0`: `Validate advanced investigations` SUCCESS и `Deploy Last Build lab preview` SUCCESS на commit `b19cb780c863af3fa58802e9fef7885a79dbe3d0`. Mobile/desktop Chrome smoke фиксирует отсутствие blocking overlay, наличие ровно одного ведущего действия и загрузку `office-hero-v2.webp`; live desktop review подтверждает видимую кнопку и сюжетную иерархию в первом viewport.

### Plain-language gate после подросткового теста `0.5.1`

Тест с игроком 14 лет показал: красивая обложка `0.5.0` всё ещё не объясняла людям вне IT, что такое «Последняя сборка», что именно произошло и что должен делать игрок. Это критическая ошибка первого знакомства, а не недостаток самого подростка.

**A — реализовано обязательно:**
- первым определением стало «Интерактивный детектив»;
- основная завязка сформулирована без жаргона: украдена секретная копия новой компьютерной игры;
- термин «сборка» оставлен только в названии дела и сразу расшифрован как готовая версия игры;
- роль сформулирована прямо: «Вы — следователь»;
- задача названа действиями игрока: изучить улики, поговорить с участниками, доказать виновного;
- первая CTA использует «Начать» или «Продолжить» и называет конкретное действие;
- разделы переименованы в «Начало / Улики / Участники / Обвинение»;
- ниже обложки добавлена короткая инструкция из трёх шагов.

Постоянное правило для всех следующих дел: название, профессиональный жаргон и атмосферная фраза не могут заменять простое объяснение продукта, события, роли игрока и ближайшего действия. Проверять первый экран на человеке, который не знает сюжет и не работает в тематической области.

QA `0.5.1`: `Validate advanced investigations` SUCCESS и `Deploy Last Build lab preview` SUCCESS на commit `3cf5905622d1dec16ceaf970bf0e52425815688a`. Smoke на mobile/desktop теперь явно проверяет пять смыслов: формат игры, понятную завязку, расшифровку термина, роль игрока и plain-language навигацию.

### People-first interview gate `0.5.2`

Ревизия экрана «Первичные показания» показала, что три кратких алиби появлялись раньше, чем игрок понимал, кто эти люди. Английские должности `operations / technical lead` и идентификатор `DEMO-04` заставляли расшифровывать внутренний жаргон вместо знакомства с участниками.

**A — реализовано обязательно:**
- все должности и связи с делом сформулированы по-русски и обычными словами;
- у каждого персонажа появились отдельные поля «связь с делом» и «за что отвечает»;
- `DEMO-04` при первом появлении объясняется как общий компьютер в переговорной для презентации игры;
- `T-17`, `RELEASE` и `ORBIT-2` при первом появлении получают смысловые определения;
- ведущая кнопка теперь предлагает «опросить участников», а не пассивно «прочитать показания».

**B — реализовано как reusable renderer:**
- общий список заменён протоколом первичных опросов;
- слева находятся три пронумерованные вкладки с портретами, справа — один выбранный человек;
- каждый опрос начинается с авторского вопроса следователя «представьтесь и объясните, как вы связаны с проектом»;
- только после ответа о личности и роли задаётся отдельный вопрос о событиях вечера;
- мобильный экран использует три компактные горизонтальные вкладки и один последовательный transcript.

**C — не делать сейчас:** озвучку трёх опросов, видео и свободный ИИ-допрос каждого персонажа. Нужный эффект живого знакомства уже достигается авторским двухступенчатым transcript без новой инфраструктуры, стоимости и риска выдуманных фактов.

Постоянное правило: алиби не является знакомством с персонажем. До первого проверяемого утверждения игрок должен понять, кто перед ним, чем человек занимается, как связан с делом и почему вообще мог оказаться рядом с событием. Внутренний код допустим как реалистичная маркировка улики, но не как замена человеческому объяснению.

QA `0.5.2`: локальные graph/eval тесты SUCCESS; authored classifier 45/45. `Validate advanced investigations` run 98 и `Deploy Last Build lab preview` run 33 — SUCCESS на commit `dd7bf0ab9f3e8cfe5bc6a60fd1cab5afd57f032b`. Live review подтвердил переключение трёх опросов, русский профиль каждого человека и расшифровку `DEMO-04` до алиби Тимура.
