...
ensureDir(generated);
const index={schemaVersion:3,sourceCommit,mode,totalCases:100,freeCount:15,premiumCount:85,seoNativePilotCount:lib.meta.filter(item=>item.seoNative).length,cases:lib.meta,freeCases:lib.freeMeta,collections:lib.collections};
...
const report={sourceCommit,mode,packages:lib.assets.length,sourceEntries:lib.sourceEntries,deprecatedIds:lib.deprecatedCount,totalCases:100,freeCases:15,premiumCases:85,seoNativePilotCases:lib.meta.filter(item=>item.seoNative).length,playablePages:editorial?100:15,lockedPages:editorial?0:85,indexableUrls:urls.length,seoLandingPages:seoSlugs.length,witnessEnhancedPages};
