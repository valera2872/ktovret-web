...
function hubPage(kind){
  ...
  const root='../';
  const selected=kind==='main'?puzzles.filter(p=>['001','006','009','014'].includes(p.n)):kind==='adult'?puzzles.filter((_,i)=>[0,1,2,3,4,8,10,11,13,16,17,18].includes(i)):puzzles;
  ...
  const seoCopy=kind==='main'
    ?`<h2>От коротких головоломок — к Expert</h2><p>На этой странице сначала доступны быстрые игровые задачи разных типов. Ниже показаны четыре примера Expert — более длинных головоломок с единственным проверяемым решением.</p><p><a href="../logicheskie-zadachi/">Открыть все ${puzzles.length} Expert-головоломок →</a> Если нужен отдельный взрослый формат, смотрите <a href="../zagadki-na-logiku-dlya-vzroslyh/">загадки на логику для взрослых с ответами</a>.</p>`
    :kind==='adult'
  ...
  const categories=kind==='main'?[]:[...new Set(puzzles.map(p=>p.category))].slice(0,10);
  return `<!doctype html><html lang="ru"><head>${head({title:cfg.title,description:cfg.description,canonical:url(`${cfg.slug}/`)})}${styles(root)}</head><body class="logic-page">${header(root)}<main class="logic-wrap"><section class="logic-seo-hero"><p class="logic-kicker">Mystery Logic · Expert</p><h1>${esc(cfg.h1)}</h1><p>${intro}</p><div class="logic-actions"><a class="logic-btn logic-btn-primary" href="#puzzles">Выбрать головоломку</a><a class="logic-btn logic-btn-ghost" data-telegram-cta="seo-hero" href="https://t.me/mysterylogic" target="_blank" rel="noopener">Новые задачи в Telegram →</a></div>${subnav(root,kind)}<div class="logic-seo-proof"><div><strong>${puzzles.length} задач</strong><span>разные механики</span></div><div><strong>Бесплатно</strong><span>без регистрации</span></div><div><strong>Expert Gate</strong><span>единственность доказана</span></div><div><strong>С ответами</strong><span>разбор спрятан до запроса</span></div></div></section><section class="logic-section" id="puzzles"><div class="logic-section-head"><div><p class="logic-kicker">${kind==='main'?'Следующий уровень':'Коллекция'}</p><h2>${kind==='adult'?'Сложные задачи для самостоятельного решения':kind==='main'?'4 задачи для знакомства с Expert':'Выберите механику'}</h2></div><p${kind==='main'?'':' data-expert-progress data-expert-total="20"'}>${kind==='main'?`Полный каталог — ${puzzles.length} задач`:'0 из 20 решено'}</p></div>${categories.length?`<div class="logic-filter-row">${categories.map(c=>`<a href="#puzzles">${esc(c)}</a>`).join('')}</div>`:''}<div class="logic-library">${selected.map(p=>card(p,root)).join('')}</div><div class="logic-seo-copy">${seoCopy}</div></section><section class="logic-section"><div class="logic-community"><div><p class="logic-kicker">Mystery Logic в Telegram</p><h2>Новая головоломка — сначала без ответа.</h2><p>Публикуем задачу, собираем версии и только потом разбираем решение. На сайте остаётся полный архив.</p></div><a class="logic-btn logic-btn-primary" data-telegram-cta="seo-community" href="https://t.me/mysterylogic" target="_blank" rel="noopener">Перейти в @mysterylogic</a></div></section></main>${footer(root)}${scripts(root)}</body></html>`;
}
