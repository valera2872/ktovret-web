...
function patchSeo(html){
  const title='«Кто врёт?» — 100 детективных дел онлайн';
  const description='«Кто врёт?» — 100 коротких детективных дел с доказуемым ответом. 15 дел бесплатно, ещё 85 открываются одной покупкой за 199 ₽.';
...
function header(){
  const nav=[['Главная','../'],['Архив дел','../dela/'],['Как это работает','#how'],['Первый том','../tom-1/'],['О нас','#about']];
...
function accessStrip(){
  return `<section class="ref-access-strip ref-who-access" aria-label="Что входит в Кто врёт"><div class="ref-access-stat"><span class="ico">♧</span><strong>15 дел</strong><span>бесплатно</span></div><div class="ref-access-stat"><span class="ico">▣</span><strong>85 дел</strong><span>в полном первом томе</span></div><div class="ref-access-stat"><span class="ico">₽</span><strong>199 ₽</strong><span>разовая покупка</span></div><div class="ref-access-buy"><a class="ref-btn ref-btn-primary" href="../delo/chetyre-vhoda-v-arhiv/">Открыть первое дело →</a><small>Без регистрации · бесплатно</small></div></section>`;
}
function playModes(){
...
function archiveSnapshot(cases){
  ...
  const links=[...free,...paid].map((item,index)=>{
    const href=item.access==='premium'?'../tom-1/':`../${item.path}`;
...
function seoCopy(){
  return `<section class="ref-who-seo" id="about"><div><p class="ref-kicker">Серия «Кто врёт?»</p><h2>100 коротких детективных дел с доказуемым ответом</h2></div><div class="ref-who-seo-copy"><p>Перед вами несколько версий одного события. Правильный ответ определяется не интуицией, а противоречием с материалами дела: временем, маршрутом, доступом, последовательностью событий или другим проверяемым условием.</p><p>Формат рассчитан на короткую игровую сессию: можно разгадывать дело самостоятельно, сравнивать версии вдвоём или читать условие вслух семье. Начните с <a href="../dela/">15 бесплатных расследований</a>. Если формат понравится, <a href="../tom-1/">полный первый том</a> открывает ещё 85 дел за 199 ₽ одной покупкой, без подписки.</p><p>Другие подборки Mystery Logic: <a href="../detektivnye-igry-onlayn/">детективные игры онлайн</a>, <a href="../detektivnye-zagadki-s-otvetami/">детективные загадки с ответами</a> и <a href="../logicheskie-detektivnye-zadachi/">логические детективные задачи</a>.</p></div></section>`;
}
...
export function applyStorefrontV4Who(siteRoot,cases){
  ...
  html=patchSeo(html);
  const required=['data-storefront-v4-who','data-reference-asset="who-approved-art"','data-reference-asset="archive-grid"','data-who-play-modes','15 дел','85 дел','199 ₽','Вслух с семьёй','Как проходит расследование','storefront-v4-who.css'];
