...
function deprecatedIds(root){
 ...
 const start=source.indexOf('_deprecatedCaseIds');
...
export function loadLibrary(sourceRoot,sourceCommit){
 ...
 for(const rel of assets){
  ...
  for(const item of bundle.cases||[]){sourceEntries+=1;if(deprecated.has(item.id))continue;if(ids.has(item.id))throw new Error(`Повтор активного ID: ${item.id}`);ids.add(item.id);active.push({...item,__asset:rel});}
 }
 ...
 const cases=[...free,...premium].map((item,index)=>{
  const number=String(index+1).padStart(3,'0'),saved=known.get(item.id);let slug=saved?.[0]||`${number}-${slugify(item.title)}`;if(used.has(slug))slug+=`-${number}`;used.add(slug);
  const set=setFor(item),characters=(item.characters||[]).filter(value=>value?.id&&!String(value.id).startsWith('__'));
  const correctOptionId=item.explanation?.correctOptionId||item.answerStages?.[0]?.correctOptionIds?.[0]||'';
  if(!item.id||!item.title||!item.intro||!item.explanation?.fullReason)throw new Error(`Неполное дело ${item.id||item.__asset}`);
  if(characters.length<2||!characters.some(value=>value.id===correctOptionId))throw new Error(`Неверный ответ в ${item.id}`);
  return{...item,number,slug,path:`delo/${slug}/`,storageKey:saved?.[1]||`ktovret:web:v5:${item.id}`,access:set.isPremium===false?'free':'premium',set:{id:set.id,title:set.title||'Расследования',description:set.description||'',order:Number(set.order??999),isListed:set.isListed!==false},characters,correctOptionId};
 });
 const meta=cases.map(item=>({id:item.id,number:item.number,title:item.title,difficulty:item.difficulty||'Среднее',category:item.category||'Логика',logicType:item.logicType||item.category||'Логическое противоречие',setId:item.set.id,setTitle:item.set.title,setOrder:item.set.order,setListed:item.set.isListed,access:item.access,path:item.path,storageKey:item.storageKey,witnessCount:item.characters.length,estimatedMinutes:estimate(item.difficulty),dailyEligible:item.dailyEligible===true}));
 return{sourceCommit,assets,sourceEntries,deprecatedCount:deprecated.size,cases,meta,freeMeta:meta.filter(item=>item.access==='free')};
}
