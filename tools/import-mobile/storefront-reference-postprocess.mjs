...
function replaceHeader(html,root,active=''){
  const nav=[['Главная',`${root}`,active==='home'],['Архив дел',`${root}dela/`,active==='archive'],['Как это работает',`${root}#method`,false],['Тома',`${root}tom-1/`,active==='volume'],['О нас',`${root}#about`,false]];
...
function footer(root){return `<footer class="ref-footer ref-wrap"><span>© 2026 Mystery Logic</span><span><a href="${root}dela/">Архив дел</a><a href="${root}detektivnye-igry-dlya-dvoih/">Для двоих</a><a href="${root}tom-1/">Тома</a><a href="${root}offer/">Условия</a></span></footer>`;}
...
function accessStrip({button=true,root='../'}={}){return `<section class="ref-access-strip"><div class="ref-access-stat"><span class="ico">♧</span><strong>10 дел</strong><span>бесплатно</span></div><div class="ref-access-stat"><span class="ico">▣</span><strong>50 + 50</strong><span>в двух томах</span></div><div class="ref-access-stat"><span class="ico">₽</span><strong>299 ₽</strong><span>оба тома</span></div><div class="ref-access-buy">${button?`<button class="ref-btn ref-btn-primary" type="button" data-volume-buy disabled aria-label="Выбрать платный том">Выбрать томы →</button><small data-volume-payment-note>Том 199 ₽ · оба 299 ₽ · без подписки</small>`:`<a class="ref-btn ref-btn-primary" href="${root}tom-1/">Выбрать томы →</a><small>Том 199 ₽ · оба 299 ₽ · без подписки</small>`}</div></section>`;}
...
function archiveGrid(items,{premiumSamples=false,root='../'}={}){
  ...
  if(premiumSamples){
    ...
    cards=[...free.map((x,i)=>caseCard(x,i+1,'Бесплатно',false,root)),...paid.map((x,i)=>caseCard(x,i+3,x.productId==='volume2'?'В Том II':'В Том I',true,root))];
  }else cards=items.filter(x=>x.access==='free').slice(0,10).map((x,i)=>caseCard(x,i+1,'Бесплатно',false,root));
...
function catalogCompatibility(cases){
  const free=cases.filter(x=>x.access==='free').slice(0,10);
  ...
  return `<div class="ref-compat" aria-hidden="true"><section data-catalog-progress><strong data-catalog-progress-text>0 из 10 бесплатных дел раскрыто</strong><span data-catalog-progress-fill></span><a data-catalog-continue href="../${esc(free[0]?.path||'dela/')}">Следующее нераскрытое дело</a><button data-catalog-random type="button">Случайное дело</button></section><input id="case-search" type="search"><select id="case-difficulty"><option value="all">Все</option></select><select id="case-category"><option value="all">Все</option></select><input id="case-unsolved" type="checkbox">${cards}<article class="volume-archive-card"><h3>Два платных тома</h3><p>Ещё 100 дел — по 50 в каждом томе</p></article></div>`;
}
function volumeCompatibility(){return `<div class="ref-compat" aria-hidden="true"><h1>Тома «Кто врёт?» — 100 платных детективных задач</h1><p>10 стартовых дел доступны бесплатно. Том I и Том II содержат по 50 расследований. Для вопросов об оплате и восстановлении доступа: support@mysterylogic.com</p><article class="volume-archive-card"><h3>Тематический архив</h3></article></div>`;}

function volumeMain(cases){return `<main class="ref-main ref-wrap"><section class="ref-archive-hero"><div class="ref-archive-copy"><p class="ref-kicker">Два тома</p><h1>100 платных детективных дел</h1><p>Том I — 50 расследований за 199 ₽. Том II — ещё 50 за 199 ₽.<br>Комплект двух томов — 299 ₽. 10 стартовых дел доступны бесплатно.</p></div><img class="ref-archive-art" src="../assets/reference-archive-hero.svg" alt="Тома Mystery Logic с папками дел и доказательствами" width="562" height="385"></section>${accessStrip({button:true})}${archiveToolbar()}${archiveGrid(cases,{premiumSamples:true})}${whyBlock()}${banner()}${volumeCompatibility()}</main>`;}
function catalogMain(cases){return `<main class="ref-main ref-wrap"><section class="ref-archive-hero"><div class="ref-archive-copy"><p class="ref-kicker">Открытый архив</p><h1>Детективные дела</h1><p>10 полноценных расследований доступны бесплатно. Решайте их в любом порядке, сохраняйте прогресс и переходите к двум платным томам, когда захотите продолжить.</p></div><img class="ref-archive-art" src="../assets/reference-archive-hero.svg" alt="Архив дел Mystery Logic" width="562" height="385"></section>${accessStrip({button:false})}${archiveToolbar()}${archiveGrid(cases,{premiumSamples:false})}${whyBlock()}${banner()}${catalogCompatibility(cases)}</main>`;}
