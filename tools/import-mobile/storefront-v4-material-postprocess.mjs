...
function materialCaseCard(item,index,{root='../'}={}){
  ...
  const href=locked?`${root}tom-1/?product=${encodeURIComponent(item.productId||'volume1')}`:`${root}${String(item.path||'').replace(/^\/+/, '')}`;
  const [pos,x,y]=archivePositions[index]||archivePositions[index%8];
  const number=String(index+1).padStart(2,'0');
  const badge=locked?(item.productId==='volume2'?'В Том II':'В Том I'):'Бесплатно';
