...
const FIXES=`storefront-v3-fixes.css?v=${VERSION}`;

const addStyle=(html,href,marker)=>html.includes(marker)?html:html.replace('</head>',`  <link rel="stylesheet" href="${href}">\n</head>`);
const addV3Styles=(html,prefix)=>addStyle(addStyle(html,`${prefix}${STYLE}`,STYLE),`${prefix}${FIXES}`,FIXES);
...
function patchHome(siteRoot){
  ...
  html=addClass(addV3Styles(html,'./assets/'));
