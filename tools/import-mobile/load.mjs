...
export function loadLibrary(sourceRoot,sourceCommit){
 ...
 const sets=new Map(),raw=[];
 for(const rel of assets){const bytes=fs.readFileSync(path.join(sourceRoot,rel));const text=rel.endsWith('.gz')?zlib.gunzipSync(bytes).toString('utf8'):bytes.toString('utf8');const bundle=JSON.parse(text);for(const set of bundle.sets||[])sets.set(set.id,set);for(const item of bundle.cases||[])raw.push({...item,__asset:rel});}
 if(raw.length!==100)throw new Error(`Ожидалось 100 дел, найдено ${raw.length}`);
 if(new Set(raw.map(x=>x.id)).size!==100)throw new Error('Повторяющиеся ID');
 ...
 if(free.length!==15||premium.length!==85)throw new Error(`Неверное разделение: ${free.length}/${premium.length}`);
 const used=new Set();
 ...
 return{sourceCommit,assets,cases,meta,freeMeta:meta.filter(x=>x.access==='free')};
}
