...
const HASHES={
  ...
  archiveGrid:'3b19ea14f7a56bd0468a36eae333642fa3509335778163e464b611e2d27cf42b',
  homeLower:'7068a5cc516fd4f80205464831e2caf1ba6487babd0bedd896f4eba7e83c92cf',
};
...
function assertWebp(bytes,label){if(bytes.subarray(0,4).toString()!=='RIFF'||bytes.subarray(8,12).toString()!=='WEBP')throw new Error(`${label} is not WebP`);}
function assertHash(bytes,expected,label){const digest=crypto.createHash('sha256').update(bytes).digest('hex');if(digest!==expected)throw new Error(`${label} checksum mismatch: ${digest}`);}
function writeVerifiedAsset(siteRoot,outName,bytes,expected,label){assertWebp(bytes,label);assertHash(bytes,expected,label);const out=path.join(siteRoot,'assets',outName);fs.writeFileSync(out,bytes);return out;}
function rebuildTextChunks(siteRoot,{pattern,expected,outName,label}){
  const dir=path.join(siteRoot,'assets','reference-parts');
  const files=fs.readdirSync(dir).filter(name=>pattern.test(name)).sort();
  if(!files.length)throw new Error(`${label} chunks missing`);
  const chunks=[];
  for(const name of files){
    const lines=fs.readFileSync(path.join(dir,name),'utf8').split(/\r?\n/).map(line=>line.trim()).filter(Boolean);
    for(const line of lines)chunks.push(Buffer.from(line,'base64'));
  }
...
function ensureVolumeCheckoutScripts(html){
  let out=html;
  if(!out.includes('paid-access-config.js')) out=out.replace('</body>','<script src="../assets/paid-access-config.js?v=1.15.0"></script></body>');
  if(!out.includes('volume-storefront.js')) out=out.replace('</body>','<script src="../assets/volume-storefront.js?v=1.15.0"></script></body>');
  return out;
}
