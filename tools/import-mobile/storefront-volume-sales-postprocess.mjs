...
function premiumArchives(cases){
  ...
  if(premium.length!==85) throw new Error(`volume sales: expected 85 premium cases, got ${premium.length}`);
  ...
  return `<section class="ref-volume-archives-v3" data-volume-premium-archives><div class="ref-volume-section-head"><p class="ref-kicker">Продолжение расследований</p><h2>85 дел в тематических архивах</h2><p>Не бесконечная лента карточек, а полный архив: разные типы противоречий, показаний, временных линий и скрытых связей.</p></div><div class="ref-volume-archives-grid">${cards}</div></section>`;
}
...
function closingCta(){
  return `<section class="ref-volume-close-v4" data-volume-closing-cta><div class="ref-volume-close-copy"><p class="ref-kicker">Полный доступ</p><h2>Расследование продолжается.</h2><p>Ещё 85 дел уже собраны в архив. Одна покупка — ${PRICE_RUB} ₽, без подписки и повторных списаний.</p></div><div class="ref-volume-close-action"><strong>85 новых дел <span>за ${PRICE_RUB} ₽</span></strong><a class="ref-btn ref-btn-primary" href="#volume-access" data-volume-scroll-buy>Получить 85 дел за ${PRICE_RUB} ₽ ↑</a><a href="../dela/">Сначала попробовать бесплатно</a></div></section>`;
}
...
function patchVolume(html,cases){
  ...
  out=out.replace(/>[^<]*(?:Первый том|том)[^<]*→<\/summary>/i,`>Получить 85 дел за ${PRICE_RUB} ₽ →</summary>`);
  ...
  if(!out.includes('data-volume-sales-v3')||!out.includes('data-free-case-count="15"')||!out.includes('data-volume-premium-archives')||!out.includes('id="volume-access"')||!out.includes(`Получить 85 дел за ${PRICE_RUB} ₽`)||!out.includes('data-volume-closing-cta')||!out.includes('data-volume-scroll-buy')) throw new Error('volume sales 15/85 patch failed');
  if(out.includes('ref-volume-free-head')||out.includes('ref-volume-included-stats')||out.includes('class="ref-case-grid ml-material-archive ref-free-grid"')) throw new Error('volume sales: duplicate stats or catalog grid leaked into sales page');
  return out;
}
