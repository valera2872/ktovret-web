...
export function applyLogicSitewide(siteRoot){
  ...
  let pages=0,navPatched=0,legacyLinksRewritten=0,homeStyled=0;
  ...
  const walk=(dir)=>{
    for(const entry of fs.readdirSync(dir,{withFileTypes:true})){
      ...
      if(!html.includes('data-logic-sitewide')) html=html.replace(/<\/body>/i,`<script data-logic-sitewide src="${scriptSrc}?v=${VERSION}" defer></script>\n</body>`);

      if(path.resolve(file)===path.join(root,'index.html')&&html.includes('data-logic-home-launch')){
        const styleHref=relativeAsset(dir,styleFile);
        if(!html.includes('logic-sitewide.css')) html=html.replace(/<\/head>/i,`<link rel="stylesheet" href="${styleHref}?v=${VERSION}">\n</head>`);
        homeStyled=1;
      }
