...
const titles = new Set(); let materialCount = 0;
for (const [index, stage] of data.stages.entries()) {
  ...
  for (const role of ['investigator', 'analyst']) {
    ...
    for (const material of materials) {
      materialCount += 1; expect(material.title && material.tag && material.type, `stage ${stage.id}/${role} contains an incomplete material`);
      expect(!titles.has(material.title), `duplicate evidence title: ${material.title}`); titles.add(material.title);
...
for (const question of data.final.questions) { const values = new Set(question.options.map(([value]) => value)); expect(values.has(question.answer), `answer for “${question.id}” is not present in its options`); expect(expectedAnswers.get(question.id) === question.answer, `canonical answer changed for “${question.id}”`); }
...
const handoffs = [['Следователь получает L-409 от Аналитика', stageRole(1,'analyst'),'L-409'],['Аналитик получает H-409 от Следователя', stageRole(1,'investigator'),'H-409'],['Следователь получает HK-44 от Аналитика', stageRole(3,'analyst'),'HK-44'],['Аналитик получает BR-220 от Следователя', stageRole(3,'investigator'),'BR-220']];
for (const [label, source, token] of handoffs) expect(source.includes(token), `${label}: source token is absent`);
for (const [label, source, token] of [['stage1 investigator must need Analyst for L-409',stageRole(1,'investigator'),'L-409'],['stage1 analyst must need Investigator for H-409',stageRole(1,'analyst'),'H-409'],['stage3 investigator must need Analyst for HK-44',stageRole(3,'investigator'),'HK-44'],['stage3 analyst must need Investigator for BR-220',stageRole(3,'analyst'),'BR-220']]) expect(!source.includes(token), `${label}: partner-only token leaked into local evidence`);

const stage1 = JSON.stringify(data.stages[0]);
for (const marker of ['SVC-407','HK-44','LOADING-B1','цифру 9','служебный лифт']) expect(!stage1.includes(marker), `stage 1 reveals escape mechanism too early: ${marker}`);
...
for (const marker of ['SVC-407','HK-44','01:18:41','служебный лифт']) expect(!stage2.includes(marker), `stage 2 reveals exact requested route too early: ${marker}`);
for (const marker of ['Нужны журналы доступа','нужен отдельный срочный запрос к системе служебного доступа','Точный журнал служебного доступа пока не запрошен']) expect(!stage2.includes(marker), `stage 2 telegraphs operational choice: ${marker}`);
...
for (const marker of ['HK-44 у меня','Футляр BR-220 войдёт','В аэропорту разделимся','масса в багажном отсеке']) expect(!stage3.includes(marker), `stage 3 contains over-explicit or implausible clue: ${marker}`);
...
for (const marker of ['D-2147','Q7-29','RB-17','4F-7719','special:2317','Вера','Илья']) expect(!allStory.includes(marker), `stale material from another case remains: ${marker}`);
for (const fragment of ['чужой служебный код','подтверждает невозможную','только у физического','собирали заранее','вызвали осознанно','в тележке был футляр','общий риск разоблачения','не мог участвовать','проходит весь маршрут','уезжает с двумя людьми','согласовывали время']) expect(![...titles].some((title) => title.toLowerCase().includes(fragment)), `evidence title gives away deduction: ${fragment}`);
...
for (const marker of ['noindex,follow','case407-catalog','room-407-evidence.webp','href="407/"','case-407-evidence-v3.css','case-407-evidence-finalize.js',"const VERSION = '1.5.2'"]) expect(postprocess.includes(marker), `page generator is missing: ${marker}`);
