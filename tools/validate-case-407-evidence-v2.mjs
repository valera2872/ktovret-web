...
for (const marker of ['case-407-evidence-v2.css', 'case-407-evidence-v3.css', 'case-407-evidence-v2.js', "const VERSION = '1.5.1'"]) expect(postprocess.includes(marker), `generated case page does not load ${marker}`);
expect(!postprocess.includes('case-407-evidence-v2-hydrate.js'), 'removed hydration layer is still loaded');
...
expect(js.includes("details.appendChild(factsNode)"), 'fact chips must be collapsed behind interpretation details');
expect(js.includes("details.appendChild(stampNode)"), 'verdict stamp must be collapsed behind interpretation details');
...
console.log(JSON.stringify({ case: data.title, rendererBytes: Buffer.byteLength(js), stylesheetBytes: Buffer.byteLength(css) + Buffer.byteLength(overrides), visualLanguages: 15, materializedEvidence: 18, roleSafeRegistry: true, collapsedInterpretation: true, mobileReadable: true, externalRuntimeDependencies: 0 }, null, 2));