...
const detectiveVisual = read('assets/case-407-detective-visual-v4.js');
const releaseGate = read('assets/case-407-release-gate-v1.js');
...
expect(detectiveVisual.length > 4_000, 'detective-v4 visual patch is unexpectedly small');
expect(releaseGate.length > 4_000, 'release-gate neutralizer is unexpectedly small');
...
for (const marker of ['patchManual', 'patchNetwork', 'patchCartLab', 'patchAccess', 'patchCar', 'patchChat', 'BR-220 / NS-17', 'не идентифицирует человека']) expect(detectiveVisual.includes(marker), `detective visual missing ${marker}`);
...
for (const marker of ['case-407-evidence-v2.css', 'case-407-evidence-v3.css', 'case-407-evidence-v2.js', 'case-407-evidence-finalize.js', 'case-407-detective-visual-v4.js', 'case-407-release-gate-v1.js', "const VERSION = '1.7.0'"]) expect(postprocess.includes(marker), `generated case page does not load ${marker}`);
expect(postprocess.indexOf('case-407-release-gate-v1.js') > postprocess.indexOf('case-407-detective-visual-v4.js'), 'release-gate neutralizer must load after visual/detective layers');
expect(!postprocess.includes('case-407-evidence-v2-hydrate.js'), 'removed legacy hydration layer is still loaded');

for (const marker of ['Выберите 5 опорных материалов', 'H-код таблички + реестр L-кодов · сверка оборудования', 'S-407 · последовательность кода и инструкция режима тревоги', 'NIGHT-MGR + ER-02 · команда режима камеры B1']) expect(releaseGate.includes(marker), `release-gate evidence layer missing neutral marker ${marker}`);
for (const marker of ['CAM G1 + действия Елены', 'общий план, но не личное действие Елены']) expect(!releaseGate.includes(marker), `release-gate evidence layer keeps answer-bearing marker ${marker}`);
...
expect(registrySource.includes('L-409'), 'Analyst registry must contain legacy L-409 before privacy bridge');
...
expect(!js.includes('fetch(') && !finalizer.includes('fetch(') && !detectiveVisual.includes('fetch(') && !releaseGate.includes('fetch('), 'evidence layer must not depend on external runtime fetches');
