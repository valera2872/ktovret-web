...
function oneMemoryOp(data,kind){if(!Array.isArray(data.operations)||data.operations.length!==1)throw new Error(`Expected one memory operation: ${JSON.stringify(data)}`);const op=data.operations[0];if(op.op!=='ask_witness'||op.target!=='boyfriend'||op.question!==`__ML_MEMORY__:${kind}`)throw new Error(`Unexpected memory plan: ${JSON.stringify(op)}`)}
...
const base={session_id:'ci-v18-memory',visitor_id:'ci-v18',completed:['people','interview','alibi','ballistics'],focus:'boyfriend',last_target:'boyfriend',recent_history:'',memory};
const status=await post({action:'status'});if(status.version!==4||status.upstream!=='ai-moreno-investigator-v3')throw new Error(`Unexpected v4 status: ${JSON.stringify(status)}`);
...
const statements=await post({...base,action:'interrogate',target:'boyfriend',question:'__ML_MEMORY__:statements'});
...
const confrontations=await post({...base,action:'interrogate',target:'boyfriend',question:'__ML_MEMORY__:confrontations'});
...
const contradictionPlan=await post({...base,action:'plan',command:'Какие здесь расхождения?'});oneMemoryOp(contradictionPlan,'contradictions');
const contradictions=await post({...base,action:'interrogate',target:'boyfriend',question:'__ML_MEMORY__:contradictions'});
if(contradictions.mode!=='memory_recall'||!String(contradictions.reply).includes('сне в кресле')||!String(contradictions.reply).includes('спрятал оружие в кресле'))throw new Error(`Contradiction memory failed: ${JSON.stringify(contradictions)}`);
for(const text of [statements.reply,confrontations.reply,contradictions.reply])if(/виновен|ключевая улика|правильный ответ/i.test(String(text)))throw new Error(`Solution cue leaked into memory: ${text}`);
