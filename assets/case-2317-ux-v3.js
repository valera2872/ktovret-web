(() => {
  ...
  const patchHandoff = (section) => {
    ...
    if (s === 3 && r === 'creator') {
      if (!done) setText(copy, 'У Аналитика есть ID метки технического шлагбаума. Получите его и запросите исходную запись сервисной камеры за минуту до открытия.');
      if (input && input.placeholder !== 'ID сервисной метки') input.placeholder = 'ID сервисной метки';
      if (done) {
        setText(copy, 'CAM-S2 · 23:30:52: Роман Белов лично входит в техническую зону и садится за руль автомобиля Веры. 23:31:44 шлагбаум открывается RB-17. Камера фиксирует лицо Романа до начала поездки; метка используется только как дополнительное совпадение.');
        setText(result, 'CAM-S2: Роман лично у автомобиля');
      }
    }
  ...
  const patchDecision = (section) => {
    ...
    if (feedback) {
      ...
      else if (text.includes('Камера кафе подтверждает')) setText(feedback, 'Маршрут Веры и Марины подтверждается дополнительным кадром, но личность водителя автомобиля Веры всё ещё не установлена.');
      else if (text.includes('Новый пакет получен')) setText(feedback, 'Получен пакет технической зоны: RB-17 открывает шлагбаум, а исходную сервисную камеру можно запросить через обмен с напарником.');
    }
    setText(section.querySelector('.case2317-drop strong'), 'Технический шлагбаум · RB-17 · пакет CAM-S2 доступен для сверки');
  };
  ...
  const patchFinal = (scope) => {
    ...
    setText(scope.querySelector('.case2317-final-evidence > p'), 'Нужна одна независимая цепочка на каждый критический тезис: Илья, безопасность Веры и водитель её автомобиля. Косвенный факт не заменяет личную идентификацию.');
  };
  ...
  patch();
  window.ML2317DetectiveV3 = Object.freeze({ revision: '3.0', fairPlay: true, personalIdentityRequired: true, idempotentDomPatch: true });
})();