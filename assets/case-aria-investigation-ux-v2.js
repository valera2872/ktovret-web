(() => {
  ...
  const install = (data) => {
    ...
    const historyOf = (progress) => Array.isArray(progress.decisionHistory) ? progress.decisionHistory : [];
    const ensureDecisionState = () => {
      const section = root.querySelector('.casearia-decision');
      if (!section) return;
      const progress = readProgress();
      const history = historyOf(progress);
      ...
      if (!handoffDone) text = 'Сначала завершите перекрёстную сверку выше. Решение должно опираться на материалы двух ролей.';
        ...
        text = `${data.decision.feedback?.[last] || 'Эта линия не выдержала проверку.'} Линия закрыта — выберите следующую версию.`;
      }
    ...
    root.addEventListener('click', (event) => {
      ...
      if (!progress.handoffs?.[data.decision.stage]) {
        event.preventDefault(); event.stopImmediatePropagation(); ensureDecisionState(); return;
      }
      ...
      if (history.includes(choice)) return;
      progress.decisionHistory = [...history, choice];
      progress.decisionMistakes = Number(progress.decisionMistakes || 0) + 1;
      writeProgress(progress);
      try { window.MysteryLogicFunnel?.track?.('diagnostic_choice',{case_id:'coop:last-aria',choice:'coop:last-aria:decision-wrong',label:data.decision.options?.find((item)=>item.id===choice)?.title||choice,position:Number(data.decision.stage||2),attempt:progress.decisionMistakes},'coop-cognitive'); } catch {}
      ensureDecisionState();
    }, true);
