...
async function init(){if(ui.name&&!ui.name.value)ui.name.value=getStore(NAME_KEY);if(ui.guestName&&!ui.guestName.value)ui.guestName.value=getStore(NAME_KEY);ui.button.onclick=buy;if(ui.name)ui.name.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();buy()}};if(ui.guestButton)ui.guestButton.onclick=enterByCode;if(ui.guestCode)ui.guestCode.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();enterByCode()}};
