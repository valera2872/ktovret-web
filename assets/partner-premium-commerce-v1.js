...
function resetPendingOrder(){pendingOrder='';setStore(ORDER_KEY,'');setStore(REQUEST_KEY,'')}
...
async function paymentStatus(orderId){if(!orderId)return false;const token=getStore(ACCESS_KEY);if(!token)return false;const {response,data}=await post(PAYSTATUS,{orderId},token);if(response.ok&&data.entitled){setEntitled(true);setStore(ORDER_KEY,orderId);return true}if(response.status===404){resetPendingOrder();return false}if(!response.ok)throw new Error(data.error||`HTTP ${response.status}`);const status=String(data.status||'').toLowerCase();if(['canceled','failed','refunded'].includes(status)){resetPendingOrder();return false}pendingOrder=orderId;return false}
