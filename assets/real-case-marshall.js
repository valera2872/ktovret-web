(()=>{
  ...
  const validateChoice=(screen)=>{
    ...
    if(value===screen.correct){
      award(screen.id,screen.points);
      state.answers[`${screen.id}:feedback`]='Позиция зафиксирована: она не закрывает альтернативы раньше, чем это позволяют материалы.';
    }else{
      state.answers[`${screen.id}:feedback`]='Позиция зафиксирована. Она делает более сильный вывод, чем текущий файл поддерживает; это отражено в доказательной оценке.';
    }
    state.answers[`${screen.id}:feedbackType`]='';
    save();
    return true;
  };
