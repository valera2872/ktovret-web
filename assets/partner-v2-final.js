(() => {
  ...
  const contradictionText = (code) => ({
    ...
    ORGANIZER_LACKS_PREPARATION_ACCESS: 'Ваша версия объясняет исполнение, но не объясняет подготовку ТК-0, доступ к параметрам груза, создание временного окна и коммерческий след внешнего покупателя.'
  ...
  const categoryText = (category) => ({
    ...
    motive_egress: 'Не доказана маршрутная половина мотива: нужен след заранее подготовленного вывоза настоящего CAXU к внешнему получателю.',
    motive_market: 'Не доказана коммерческая половина мотива: нужен независимый след покупателя, который заранее искал именно восемь модулей нужной серии.'
