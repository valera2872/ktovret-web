...
function showView(name){state.view=name;Object.entries(views).forEach(([k,node])=>node.hidden=k!==name);saveState();window.scrollTo({top:0,left:0,behavior:'instant'})}
...
$('[data-action="start"]').addEventListener('click',()=>{renderEvidence();renderSuspects();renderRoom();renderNotes();showView('workspace');if(window.matchMedia('(hover:hover) and (pointer:fine)').matches)setTimeout(()=>$('#aid-question')?.focus({preventScroll:true}),200)});
