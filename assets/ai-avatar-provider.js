(()=>{"use strict";
const DEFAULT_CONFIG={enabled:false,provider:"heygen",sessionEndpoint:"/functions/v1/ai-avatar-session",suspects:{}};
...
class HeyGenLiveAvatarProvider extends AvatarProvider{
  async connect({video,suspectId}={}){
    ...
    const endpoint=this.config.sessionEndpoint||DEFAULT_CONFIG.sessionEndpoint;
    const response=await fetch(endpoint,{method:"POST",headers:{"content-type":"application/json"},credentials:"same-origin",body:JSON.stringify({provider:"heygen",avatar_id:avatarId,suspect_id:this.suspectId,mode:"lite"})});
  ...
  async setSuspect(id){
    if(id===this.suspectId)return;await this.disconnect();this.suspectId=id||"";
  }
  async setStage(stage){
    await super.setStage(stage);if(this.connected&&typeof this.client?.setStage==="function")await this.client.setStage(this.stage)
  }
  async speak(text){
    if(!this.connected||!text)return false;await this.client.speak(String(text),{stage:this.stage,suspectId:this.suspectId});return true
  }
