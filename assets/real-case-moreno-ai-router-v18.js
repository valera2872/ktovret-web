...
function buildMemory(){let state={};try{state=JSON.parse(localStorage.getItem(KEY)||'{}')||{}}catch{}const entries=[];for(let i=0;i<(Array.isArray(state.journal)?state.journal:[]).length;i++){const turn=state.journal[i]||{};for(const item of Array.isArray(turn.items)?turn.items:[]){const title=norm(item?.title),body=norm(item?.body);if(!body||/^Память дела:/i.test(body)||/^Прямой допрос открыт$/i.test(title))continue;let [id,actor]=actorFromItem(title),storedBody=body;if(!id&&/^Опрос жильцов квартиры$/i.test(title)){const marker='Бойфренд старшей дочери заявил';const at=body.indexOf(marker);if(at>=0){id='boyfriend';actor='бойфренд старшей дочери';storedBody=body.slice(at)}}if(!id)continue;entries.push({turn:i+1,actor_id:id,actor,type:/^Вы предъявили\s/i.test(body)?'confrontation':'statement',command:norm(turn.command).slice(0,260),title:title.slice(0,180),body:storedBody.slice(0,1100)})}}return{entries:entries.slice(-30)}}
