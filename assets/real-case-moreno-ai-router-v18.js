(()=>{
'use strict';
const FROM='https://orknvuwknvsedjgqcfwc.supabase.co/functions/v1/ai-moreno-investigator-v2';
const TO='https://orknvuwknvsedjgqcfwc.supabase.co/functions/v1/ai-moreno-investigator-v4';
const KEY='ml-realcase-moreno-ai-v6';
const original=window.fetch.bind(window);
const norm=s=>String(s||'').replace(/\s+/g,' ').trim();
const ACTORS={
 'бойфренд старшей дочери':'boyfriend',
 'приёмная мать Patricia':'mother',
 'приемная мать Patricia':'mother',
 'старшая дочь приёмной матери':'older_daughter',
 'старшая дочь приемной матери':'older_daughter',
 'младшая дочь приёмной матери':'younger_daughter',
 'младшая дочь приемной матери':'younger_daughter',
 'бывший житель второго этажа':'second_floor_witness'
};
function actorId(name){const n=norm(name);if(ACTORS[n])return ACTORS[n];if(/бойфренд/.test(n))return'boyfriend';if(/при[её]мн.*мать/.test(n))return'mother';if(/старш.*доч/.test(n))return'older_daughter';if(/младш.*доч/.test(n))return'younger_daughter';if(/втор.*этаж|сосед/.test(n))return'second_floor_witness';return''}
function actorFromItem(title,body){let m=norm(title).match(/^Ответ:\s*(.+)$/i);if(m){const id=actorId(m[1]);if(id)return[id,norm(m[1])]}const map=[[/^Опрос бойфренда/i,'boyfriend','бойфренд старшей дочери'],[/^Опрос при[её]мной матери/i,'mother','приёмная мать Patricia'],[/^Опрос бывшего жильца второго этажа/i,'second_floor_witness','бывший житель второго этажа']];for(const [re,id,label] of map)if(re.test(norm(title)))return[id,label];if(/^Прямой допрос открыт$/i.test(norm(title))){const x=norm(body).match(/^Перед вами\s+(.+?)\./i);if(x){const id=actorId(x[1]);if(id)return[id,norm(x[1])]}}return['','']}
function buildMemory(){let state={};try{state=JSON.parse(localStorage.getItem(KEY)||'{}')||{}}catch{}const entries=[];for(let i=0;i<(Array.isArray(state.journal)?state.journal:[]).length;i++){const turn=state.journal[i]||{};for(const item of Array.isArray(turn.items)?turn.items:[]){const title=norm(item?.title),body=norm(item?.body);if(!body||/^Память дела:/i.test(body))continue;const [id,actor]=actorFromItem(title,body);if(!id)continue;entries.push({turn:i+1,actor_id:id,actor,type:/^Вы предъявили\s/i.test(body)?'confrontation':'statement',command:norm(turn.command).slice(0,260),title:title.slice(0,180),body:body.slice(0,1100)})}}return{entries:entries.slice(-30)}}
function augmentBody(body){if(!body||typeof body!=='string')return body;try{const data=JSON.parse(body);if(data&&typeof data==='object'&&(data.action==='plan'||data.action==='interrogate'))data.memory=buildMemory();return JSON.stringify(data)}catch{return body}}
window.fetch=(input,init={})=>{
 if(typeof input==='string'&&input===FROM)return original(TO,{...init,body:augmentBody(init.body)});
 if(input instanceof Request&&input.url===FROM){const request=new Request(TO,input);return original(request,init)}
 return original(input,init);
};
window.MLMorenoAIRouterV18={version:'1.8.0',from:FROM,to:TO,buildMemory};
})();
