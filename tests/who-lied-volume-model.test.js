...
assert.ok(loader.includes('TOTAL_CASES=110'),'catalog must contain 110 cases');
assert.ok(loader.includes('FREE_COUNT=10'),'exactly 10 cases must be free');
assert.ok(loader.includes('VOLUME_SIZE=50'),'each paid volume must contain 50 cases');
...
assert.ok(caseAccess.includes("LEGACY_VOLUME_ALL_PRODUCT_ID = 'legacy_volume_all'"),'case access must accept grandfathered ownership');
