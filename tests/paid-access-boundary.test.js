...
const multiProductMigration=fs.readFileSync(path.join(root,'supabase/migrations/20260906110000_multi_product_entitlements.sql'),'utf8');
...
assert.equal(catalog.totalCases,110,'Who Lied catalog must contain 110 cases');
assert.equal(premium.length,100,'premium boundary must contain 100 cases');
assert.equal(free.length,10,'free boundary must contain 10 cases');
assert.equal(volume1.length,50,'Volume I must contain 50 cases');
assert.equal(volume2.length,50,'Volume II must contain 50 cases');
assert.equal(report.paidGatewayPages,100,'all 100 locked pages need the gateway');

for(const item of premium){
  const html=fs.readFileSync(path.join(root,item.legacyPath,'index.html'),'utf8');
  assert.ok(html.includes('data-paid-case-gateway="2.0.0"'),`${item.id} needs paid gateway marker`);
  assert.ok(html.includes(`data-product-id="${item.productId}"`),`${item.id} needs owning product marker`);
  assert.ok(html.includes('data-paid-access-panel'),`${item.id} needs paid access panel`);
  assert.ok(html.includes('paid-access-client.js?v=2.0.0'),`${item.id} needs paid access client`);
  assert.ok(html.includes('paid-access-config.js?v=2.0.0'),`${item.id} needs paid access runtime config`);
  assert.ok(html.includes(`product=${item.productId}`),`${item.id} must route unpurchased users to its owning volume`);
...
assert.ok(runtime.includes("volume1:{label:'Том I',priceRub:199,caseCount:50}"),'Volume I client offer missing');
assert.ok(runtime.includes("volume2:{label:'Том II',priceRub:199,caseCount:50}"),'Volume II client offer missing');
assert.ok(runtime.includes("volume_bundle_1_2:{label:'Том I + Том II',priceRub:299,caseCount:100"),'bundle client offer missing');
...
assert.ok(storefront.includes('productId: selectedProductId'),'checkout must send selected server-known product id');
assert.ok(storefront.includes("selectProduct('volume_bundle_1_2'"),'bundle must be a selectable storefront offer');
...
assert.ok(paymentShared.includes('PRODUCT_CATALOG'),'server-side fixed product catalog missing');
assert.ok(paymentShared.includes("volume1: {"),'server-side Volume I missing');
assert.ok(paymentShared.includes("priceRub: 199"),'199 RUB server price missing');
assert.ok(paymentShared.includes("volume_bundle_1_2: {"),'server-side bundle missing');
assert.ok(paymentShared.includes("priceRub: 299"),'299 RUB bundle price missing');
assert.ok(paymentShared.includes("entitlementProductIds: ['volume1', 'volume2']"),'bundle must grant both volume entitlements');
assert.ok(paymentShared.includes('onConflict: \'token_hash,product_id\''),'multi-product entitlement upsert must use composite key');
...
assert.ok(paymentStatus.includes('entitlementProductIds: grantIds'),'status response must expose granted volume ids to its owning browser token');
...
assert.ok(multiProductMigration.includes('drop constraint if exists access_entitlements_token_hash_key'),'legacy one-product uniqueness must be removed');
assert.ok(multiProductMigration.includes('access_entitlements_token_product_uidx'),'composite entitlement uniqueness missing');
assert.ok(multiProductMigration.includes('(token_hash, product_id)'),'composite entitlement key missing');
...
if(fs.existsSync(mobileSource)){
  ...
  assert.equal(bundle.totalCases,100,'secure export must contain 100 paid cases');
  assert.equal(bundle.items.length,100,'secure export item count mismatch');
  assert.equal(bundle.products.volume1,50,'secure export Volume I count mismatch');
  assert.equal(bundle.products.volume2,50,'secure export Volume II count mismatch');
  for(const item of bundle.items){
    assert.ok(['volume1','volume2'].includes(item.product_id),`${item.case_id} has unknown product`);
...
console.log('paid access boundary passed: T-Bank / 10 public / 50 + 50 server-gated / 299 RUB bundle');
