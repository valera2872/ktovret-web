...
assert.ok(!html.includes('0 / 30 вопросов'),'visible turn budget must not appear before the player approaches the cap');
...
assert.ok(client.includes("$('[data-action=\"theory\"]').addEventListener('click',openTheory)"),'theory action must always open the player reconstruction');
...
for(const eventName of ['ai01_feedback_submitted','ai01_feedback_like','ai01_feedback_improve','ai01_feedback_more']) {
  assert.ok(analytics.includes(eventName),`feedback analytics event missing: ${eventName}`);
}
