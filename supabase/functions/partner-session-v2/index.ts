...
const MUTATING=new Set(['OPEN_EVIDENCE','PUBLISH_FINDING','ATTEMPT_DEDUCTION','PRESENT_EVIDENCE','CHALLENGE_CHARACTER','SUBMIT_LOCATION','ATTEMPT_RECONSTRUCTION','CHOOSE_PUBLICATION']);
...
async function ensureStateRow(roomId:string){let row=await loadStateRow(roomId);if(row){if(row.case_id!==FULL_PARTNER_CASE_ID)throw new Error('partner_wrong_state_case');return row}try{const created=await rest('partner_room_states?select=room_id,case_id,state,revision,entitlement_id',{method:'POST',headers:{prefer:'return=representation'},body:JSON.stringify({room_id:roomId,case_id:FULL_PARTNER_CASE_ID,state:createInitialFullPartnerState(),revision:0})});row=Array.isArray(created)?created[0]:null}catch(error){const msg=String(error);if(!msg.includes('23505')&&!msg.includes('duplicate'))throw error;row=await loadStateRow(roomId)}if(!row)throw new Error('partner_state_unavailable');return row}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    const room=await roomByCode(code);const hash=await digest(browserKey);const member=await memberContext(String(room.id),hash);let row=await ensureStateRow(String(room.id));let state=normalizeFullPartnerState(row.state);let revision=Number(row.revision)||0;
    if(MUTATING.has(actionType)&&!member.partner.joined)throw new Error('partner_partner_required');
    if(MUTATING.has(actionType)){let saved:any=null;for(let attempts=0;attempts<3&&!saved;attempts++){const next=processFullPartnerAction(state,member.role,{...body,type:actionType});saved=await compareAndSave(String(room.id),revision,next);if(saved){state=normalizeFullPartnerState(saved.state);revision=Number(saved.revision)||revision+1;break}row=await loadStateRow(String(room.id));if(!row)throw new Error('partner_state_unavailable');state=normalizeFullPartnerState(row.state);revision=Number(row.revision)||0}if(!saved)throw new Error('partner_state_conflict')}
