...
Deno.serve(async (req: Request) => {
  ...
  try {
    if (providerStatus === 'REFUNDED') {
      await finalizePartnerRefund(admin, order);
      return ok();
    }
    if (providerStatus) {
    ...
    if (['CONFIRMED', 'CANCELED', 'REJECTED', 'REVERSED', 'DEADLINE_EXPIRED'].includes(providerStatus)) {
      await refreshPartnerTbankOrder(admin, { ...order, provider_status: providerStatus });
    }
    return ok();
  } catch (error) {
    console.error('partner_tbank_webhook_error', String(error));
