...
const publicEvidence:Record<string,string>={
  ...
  E02:"С 21:27:10 до 21:35:42 камеры наблюдения служебного коридора были недоступны из-за планового перезапуска; запись в этот период не велась. Заявка создана в 20:15.",
  ...
  E05:"Журнал служебной Wi-Fi сети: контроллер фиксирует, что служебный телефон Марины оставался подключён к внутренней точке Archive-2 до 21:34:27. Во внутреннем дворике эта точка недоступна.",
...
const suspectBase:Record<string,{name:string;role:string;persona:string;facts:string[]}>= {
  marina:{name:"Марина Лебедева",role:"архивист фонда",persona:"Сдержанная, профессиональная, слегка раздражается от обвинительного тона. Не болтлива. Старается звучать уверенно и держит дистанцию. Если её ловят на подтверждённом противоречии, сначала уточняет формулировку, затем признаёт только неизбежный минимум.",facts:[
    "После 21:25 ты утверждала, что находилась во внутреннем дворике и разговаривала по своему служебному телефону.",
    ...
    "Ты действительно спрашивала Антона о точном времени планового перезапуска камер около 18:20, но не говори об этом сама до соответствующего вопроса после показания Антона."
  ]},
  anton:{name:"Антон Руденко",role:"инженер безопасности",persona:"Технический специалист, отвечает конкретно и без театральности. Когда вопрос расплывчатый, просит уточнить, какой журнал или систему проверить. Не защищает Марину и не обвиняет её — сообщает только то, что сам знает или может проверить.",facts:[
    "Ты инициировал плановый перезапуск камер наблюдения служебного коридора; заявка создана в 20:15.",
    "С 21:27 до 21:35 камеры наблюдения служебного коридора были недоступны из-за планового перезапуска. В это время ты находился в комнате контроля.",
    "Ты знаешь реестр доступа и можешь подтвердить, что E-14 принадлежит Марине и требует карту плюс персональный PIN.",
    "Около 18:20 Марина спросила у тебя точное время перезапуска камер, и ты ей его назвал.",
    "Служебные телефоны сотрудников автоматически подключаются к внутренней Wi-Fi сети, а контроллер хранит журнал подключений по точкам доступа. Это внутренний журнал архива, а не геолокация мобильного оператора.",
    "Если следователь просит проверить алиби или местонахождение Марины, ты можешь проверить журнал служебной Wi-Fi сети её телефона и объяснить, что точка Archive-2 недоступна во внутреннем дворике."
  ]},
...
const marinaAdmissions:Record<string,string>={
  ...
  "N-MARINA-WINDOW":"Ты уже признала, что заранее спрашивала Антона о точном времени планового перезапуска камер.",
  "N-MARINA-CONFESSION":"Ты уже призналась, что, пока камеры были недоступны из-за планового перезапуска, вошла в фонд и взяла письмо. От этого признания отказываться нельзя.",
  "N-MARINA-MOTIVE":"Ты уже объяснила мотив: у тебя был просроченный долг около 9 000 €, ты знала страховую оценку письма — 26 000 € — и рассчитывала продать оригинал. На следующий день документ должны были перевести в хранилище с двойным контролем, поэтому эта смена была последней удобной возможностью."
};
...
function isWindowReference(q:string){return hasAny(q,["камера","камеры","перезапуск","отключ","недоступ","окно","18:20","антон","руденко"])}
function isMotiveReference(q:string){return hasAny(q,["зачем","почему","мотив","деньг","долг","продат","продаж","стоим","цена","выгода","26 000","26000"])}
...
function isMarinaAlibiCheck(q:string){
  const target=hasAny(q,["марин","лебедев","её алиби","ее алиби","её местонах","ее местонах","была ли она","где она","двор","телефон"]);
  const verification=hasAny(q,["провер","подтверд","установ","алиби","местонахожд","где была","где наход","техническ","данн","журнал","wi-fi","wifi","вайф","сеть","телефон"]);
...
function unlock(suspect:string,q:string,evidenceId:string,discoveredNotes:Set<string>,discoveredEvidence:Set<string>,qc:Counts,resistance:ResistanceLevel){
  ...
  if(suspect==="anton"){
    ...
    if(hasAny(q,["кто знал","кто ещё знал","точное время","время перезапуска","окно перезапуска","когда камера","когда камеры","кто спрашивал","предупреждали"]))notes.push({id:"N-ANTON-WINDOW",source:"Антон · допрос",text:"Антон сообщает: около 18:20 Марина отдельно спросила у него точное время планового перезапуска камер, и он назвал ей точный период."});
    ...
    if(isMarinaAlibiCheck(q))unlockedEvidence.push("E05");
  }
  if(suspect==="lev"){
  ...
  if(suspect==="marina"){
    ...
    if(locationEstablished&&!discoveredNotes.has("N-MARINA-LOCATION"))notes.push({id:"N-MARINA-LOCATION",source:"Марина · противоречие",text:"После предъявления журнала служебной Wi-Fi сети Марина признаёт, что её первоначальная версия была неточной: до 21:34 её служебный телефон оставался внутри здания, хотя она утверждала, что разговаривала по нему во внутреннем дворике."});
    ...
    if(windowEstablished&&!discoveredNotes.has("N-MARINA-WINDOW"))notes.push({id:"N-MARINA-WINDOW",source:"Марина · уточнение",text:"Марина признаёт, что заранее спрашивала Антона о точном времени планового перезапуска камер; объясняет это рабочей необходимостью."});

    let activeNotes=new Set([...discoveredNotes,...notes.map(n=>n.id)]);
    ...
    if(!activeNotes.has("N-MARINA-CONFESSION")&&(hardClimax||adaptiveClimax)){
      const text=resistance==="hard"
        ?"После того как следователь свёл воедино ложное алиби, персональный доступ, заранее известное время недоступности камер и подтверждённые алиби остальных, Марина признаётся: пока камеры были недоступны из-за планового перезапуска, она вошла в фонд и взяла письмо."
        :"После накопленного давления и прямого обвинения Марина больше не может удерживать прежнюю версию и признаётся: пока камеры были недоступны из-за планового перезапуска, она вошла в фонд и взяла письмо.";
      notes.push({id:"N-MARINA-CONFESSION",source:"Марина · признание",text});
      activeNotes=new Set([...activeNotes,"N-MARINA-CONFESSION"]);
    }

    if(activeNotes.has("N-MARINA-CONFESSION")&&isMotiveReference(q)&&!activeNotes.has("N-MARINA-MOTIVE")){
      notes.push({id:"N-MARINA-MOTIVE",source:"Марина · мотив",text:"После признания Марина объясняет мотив: у неё был просроченный долг около 9 000 €. Она знала страховую стоимость письма — 26 000 € — и рассчитывала продать оригинал после похищения. На следующий день документ должны были перевести в хранилище с двойным контролем, поэтому эта смена была последней удобной возможностью."});
    }
  }
...
function speakingBrief(suspect:string,q:string,evidenceId:string,notes:Note[],unlockedEvidenceIds:string[],discoveredNotes:Set<string>,discoveredEvidence:Set<string>,stage:InterrogationStage,confessionThisTurn:boolean,resistance:ResistanceLevel){
  ...
  if(suspect==="marina"){
    ...
    if(activeNotes.has("N-MARINA-CONFESSION")&&!activeNotes.has("N-MARINA-MOTIVE"))facts.push("После признания не выкладывай мотив сам по себе. Если следователь прямо спрашивает зачем, почему, о деньгах, долге, цене или продаже письма, мотив можно раскрыть только если он отдельно разрешён ниже в этом brief.");
  }
...
Ответь только следующей репликой персонажа.`:q;
  ...
  if(confessionThisTurn&&!containsConfession(text))text="…Да. Письмо взяла я. Я вошла в фонд, пока камеры были недоступны из-за планового перезапуска.";
...
function checkTheory(suspect:string,reason:string,discoveredNotes:Set<string>,discoveredEvidence:Set<string>,resistance:ResistanceLevel){
  ...
  if(suspect!=="marina")return {correct:false,title:"Эта версия пока не закрывает дело",explanation:"Проверьте, есть ли у выбранного человека подтверждённая связь с входом в фонд в 21:31, опровергнутое алиби и возможность действовать в нужный период."};
  ...
  const motiveEstablished=discoveredNotes.has("N-MARINA-MOTIVE");
  ...
  if(!confessed){
    const explanation=resistance==="hard"
      ?"Вы уже собрали необходимую доказательную цепочку. Теперь сведите в одном обвинительном вопросе её ложное алиби, персональный доступ, заранее известное время недоступности камер и подтверждённые алиби остальных. Простого вопроса «это вы?» недостаточно."
  ...
  if(!motiveEstablished)return {correct:false,title:"Вы установили кто и как — но пока не зачем",explanation:"Вернитесь к Марине и выясните мотив. Для полной реконструкции недостаточно одного признания: нужно понять, что подтолкнуло её к похищению именно в эту смену."};
  const dimensions=[hasAny(normalized,["e-14","е-14","карта","pin","пин","21:31","двер","доступ"]),hasAny(normalized,["телефон","wi-fi","wifi","вайф","сеть","archive-2","двор","21:34","внутри"]),hasAny(normalized,["камера","камеры","перезапуск","отключ","недоступ","18:20","знала время","спрашивала","антон"])].filter(Boolean).length;
  const motiveMentioned=hasAny(normalized,["мотив","долг","деньг","финанс","прод","стоим","26 000","26000"]);
  if(dimensions<3||!motiveMentioned)return {correct:false,title:"Факты собраны, но в объяснении не хватает связки",explanation:"Опишите своими словами, как связаны мотив, персональный доступ, проверка местонахождения и заранее известное время недоступности камер. Простого выбора имени недостаточно."};
  return {correct:true,title:"Версия выдерживает проверку",explanation:"Цепочка закрывается независимыми линиями: E-14 с персональным PIN открывает фонд в 21:31 и принадлежит Марине; её версия о дворике опровергнута журналом служебной Wi-Fi сети; время недоступности камер она выяснила заранее; алиби Антона и Льва подтверждены. После признания Марина объяснила и мотив: просроченный долг, высокая стоимость письма и перевод документа на двойной контроль на следующий день.",reveal:"Ответственная — Марина Лебедева. Ей требовались деньги для погашения просроченного долга; зная страховую оценку письма в 26 000 €, она рассчитывала продать оригинал. На следующий день документ должны были перевести в хранилище с двойным контролем, поэтому Марина заранее выяснила время планового перезапуска камер и использовала эту смену как последнюю удобную возможность."};
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(!unlocked.notes.some(n=>n.id==="N-MARINA-CONFESSION")&&legacyDeepConfessionReady(suspect,question,history,serverTurnsBefore,resistance)){
      unlocked.notes.push({id:"N-MARINA-CONFESSION",source:"Марина · признание",text:"После долгого допроса и финального сведения доступа, местонахождения, времени недоступности камер и алиби остальных Марина признаётся: пока камеры были недоступны из-за планового перезапуска, она вошла в фонд и взяла письмо."});
    }
    const activeNotes=new Set([...discoveredNotes,...unlocked.notes.map(n=>n.id)]);
    if(suspect==="marina"&&activeNotes.has("N-MARINA-CONFESSION")&&isMotiveReference(question)&&!activeNotes.has("N-MARINA-MOTIVE")){
      unlocked.notes.push({id:"N-MARINA-MOTIVE",source:"Марина · мотив",text:"После признания Марина объясняет мотив: у неё был просроченный долг около 9 000 €. Она знала страховую стоимость письма — 26 000 € — и рассчитывала продать оригинал после похищения. На следующий день документ должны были перевести в хранилище с двойным контролем, поэтому эта смена была последней удобной возможностью."});
    }
    const stage=interrogationStage(suspect,new Set([...discoveredNotes,...unlocked.notes.map(n=>n.id)]),discoveredEvidence,qc,resistance);
