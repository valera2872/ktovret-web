...
const publicEvidence:Record<string,string>={
  ...
  E02:"С 21:27:10 до 21:35:42 камера служебного коридора проходила плановый перезапуск; запись в этот период не велась. Заявка создана в 20:15.",
...
const suspectBase:Record<string,{name:string;role:string;persona:string;facts:string[]}>= {
  ...
  anton:{name:"Антон Руденко",role:"инженер безопасности",persona:"Технический специалист, отвечает конкретно и без театральности. Когда вопрос расплывчатый, просит уточнить, какой журнал или систему проверить. Не защищает Марину и не обвиняет её — сообщает только то, что сам знает или может проверить.",facts:[
    ...
    "Во время планового перезапуска ты находился в комнате контроля.",
...
const marinaAdmissions:Record<string,string>={
  ...
  "N-MARINA-CONFESSION":"Ты уже призналась, что во время планового перезапуска камеры вошла в фонд и взяла письмо. От этого признания отказываться нельзя.",
...
function isWindowReference(q:string){return hasAny(q,["камера","перезапуск","отключ","окно","18:20","антон","руденко"])}
...
function unlock(suspect:string,q:string,evidenceId:string,discoveredNotes:Set<string>,discoveredEvidence:Set<string>,qc:Counts,resistance:ResistanceLevel){
  ...
  if(suspect==="anton"){
    ...
    if(hasAny(q,["кто знал","кто ещё знал","точное время","время перезапуска","окно перезапуска","когда камера","кто спрашивал","предупреждали"]))notes.push({id:"N-ANTON-WINDOW",source:"Антон · допрос",text:"Антон сообщает: около 18:20 Марина отдельно спросила у него точное время планового перезапуска камеры, и он назвал ей точный период."});
  ...
  if(suspect==="marina"){
    ...
    if(!activeNotes.has("N-MARINA-CONFESSION")&&(hardClimax||adaptiveClimax)){
      const text=resistance==="hard"
        ?"После того как следователь свёл воедино ложное алиби, персональный доступ, заранее известное время перезапуска камеры и подтверждённые алиби остальных, Марина признаётся: во время планового перезапуска камеры она вошла в фонд и взяла письмо."
        :"После накопленного давления и прямого обвинения Марина больше не может удерживать прежнюю версию и признаётся: во время планового перезапуска камеры она вошла в фонд и взяла письмо.";
...
Ответь только следующей репликой персонажа.`:q;
  ...
  if(confessionThisTurn&&!containsConfession(text))text="…Да. Письмо взяла я. Я вошла в фонд во время планового перезапуска камеры.";
...
function checkTheory(suspect:string,reason:string,discoveredNotes:Set<string>,discoveredEvidence:Set<string>,resistance:ResistanceLevel){
  ...
  if(!confessed){
    const explanation=resistance==="hard"
      ?"Вы уже собрали необходимую доказательную цепочку. Теперь сведите в одном обвинительном вопросе её ложное алиби, персональный доступ, заранее известное время перезапуска камеры и подтверждённые алиби остальных. Простого вопроса «это вы?» недостаточно."
  ...
  const dimensions=[hasAny(normalized,["e-14","е-14","карта","pin","пин","21:31","двер","доступ"]),hasAny(normalized,["телефон","wi-fi","wifi","вайф","сеть","archive-2","двор","21:34","внутри"]),hasAny(normalized,["камера","перезапуск","отключ","18:20","знала время","спрашивала","антон"])].filter(Boolean).length;
  const motiveMentioned=hasAny(normalized,["мотив","долг","деньг","финанс","прод","стоим","26 000","26000"]);
  if(dimensions<3||!motiveMentioned)return {correct:false,title:"Факты собраны, но в объяснении не хватает связки",explanation:"Опишите своими словами, как связаны мотив, персональный доступ, проверка местонахождения и заранее известное время перезапуска камеры. Простого выбора имени недостаточно."};
  return {correct:true,title:"Версия выдерживает проверку",explanation:"Цепочка закрывается независимыми линиями: E-14 с персональным PIN открывает фонд в 21:31 и принадлежит Марине; её версия о дворике опровергнута сетевым логом; время перезапуска камеры она выяснила заранее; алиби Антона и Льва подтверждены. После признания Марина объяснила и мотив: просроченный долг, высокая стоимость письма и перевод документа на двойной контроль на следующий день.",reveal:"Ответственная — Марина Лебедева. Ей требовались деньги для погашения просроченного долга; зная страховую оценку письма в 26 000 €, она рассчитывала продать оригинал. На следующий день документ должны были перевести в хранилище с двойным контролем, поэтому Марина заранее выяснила время планового перезапуска камеры и использовала эту смену как последнюю удобную возможность."};
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(!unlocked.notes.some(n=>n.id==="N-MARINA-CONFESSION")&&legacyDeepConfessionReady(suspect,question,history,serverTurnsBefore,resistance)){
      unlocked.notes.push({id:"N-MARINA-CONFESSION",source:"Марина · признание",text:"После долгого допроса и финального сведения доступа, местонахождения, времени перезапуска камеры и алиби остальных Марина признаётся: во время планового перезапуска камеры она вошла в фонд и взяла письмо."});
    }
