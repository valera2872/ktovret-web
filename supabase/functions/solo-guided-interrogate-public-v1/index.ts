...
const ACCESS_MODES=new Set(['demo','owned','club','admin']);
...
async function claimTurn(req:Request,sessionKey:string){return rpc('ai_detective_claim_turn',{p_session_id:`ml0512-${sessionKey.slice(0,48)}`,p_visitor_hash:await protectedHash('visitor',sessionKey),p_network_hash:await protectedHash('network',clientNetwork(req)),p_reserve_usd:RESERVE_USD,p_session_limit:SESSION_LIMIT,p_visitor_daily_limit:VISITOR_DAILY_LIMIT,p_network_daily_limit:NETWORK_DAILY_LIMIT,p_daily_budget_usd:DAILY_BUDGET_USD,p_session_rpm:SESSION_RPM,p_network_rpm:NETWORK_RPM});}
async function releaseTurn(id:string){try{await rpc('ai_detective_release_turn',{p_claim_id:id});}catch{}}
async function completeTurn(id:string,u:Usage){try{await rpc('ai_detective_complete_turn',{p_claim_id:id,p_actual_usd:u.costUsd,p_input_tokens:u.inputTokens,p_cached_input_tokens:u.cachedInputTokens,p_output_tokens:u.outputTokens});}catch(e){console.error('ml0512_public_metering_complete',String(e));}}
...
function quotaMessage(code:string){return({session_limit:'Лимит ИИ этого прохождения исчерпан.',visitor_daily_limit:'Дневной лимит ИИ этого прохождения исчерпан.',network_daily_limit:'Дневной лимит ИИ этой сети исчерпан.',daily_budget:'Дневной бюджет ИИ временно исчерпан.',session_rate_limit:'Слишком много вопросов подряд. Попробуйте немного позже.',network_rate_limit:'Слишком много запросов из этой сети. Попробуйте немного позже.'} as Record<string,string>)[code]||'ИИ-допрос временно недоступен.';}
...
Deno.serve(async(req:Request)=>{
  ...
  if(clean(body.action,30).toUpperCase()==='STATUS')return json(origin,200,{version:1,mode:'public-state-bounded-character-interrogation',caseId:CASE_ID,model:MODEL});
  const caseId=clean(body.case_id,160),rawSessionToken=clean(body.session_token,512),characterId=clean(body.character_id,80),question=clean(body.question,700);
  if(caseId!==CASE_ID||!rawSessionToken||!CHARACTER_IDS.has(characterId)||!question)return json(origin,400,{error:'invalid_request'});
  try{
    ...
    const rawMode=clean(row?.state?.entitlement,20);
    const accessMode=(ACCESS_MODES.has(rawMode)?rawMode:'demo') as 'demo'|'owned'|'club'|'admin';
    const state=normalizeStoredSoloState(row.state,runtime,accessMode);
    const ctx=speakingContext(runtime,state,characterId);
    const claim=await claimTurn(req,sessionKey);
    ...
    try{
      ...
      if(!response.ok){const detail=clean(await response.text(),300);console.error('ml0512_public_openai',response.status,detail);await releaseTurn(claimId);throw new Error('model_unavailable');}
  ...
  }catch(error){const code=clean(error instanceof Error?error.message:error,120)||'interrogation_failed';const status=code==='solo_session_not_found'?404:code.startsWith('metering_')||code==='model_unavailable'?503:400;console.error('ml0512_public_interrogation_error',code);return json(origin,status,{error:code,message:status===503?'ИИ-допрос временно недоступен. Попробуйте ещё раз.':undefined});}
});
