...
const cleanName = (value: unknown) => String(value || '').replace(/[\u0000-\u001f\u007f]/g, '').trim().replace(/\s+/g, ' ').slice(0, 32) || 'Игрок';
...
const publicMetrics = (player: Record<string, any>) => ({
  name: player.player_name, role: player.role, elapsedSeconds: player.elapsed_seconds, hintsUsed: player.hints_used,
  attempts: player.attempts, firstAnswerCorrect: player.first_answer_correct, completedAt: player.completed_at,
});
const buildView = async (admin: AdminClient, room: Record<string, any>, browserKeyHash: string) => {
  ...
  const bothJoined = Boolean(creator && guest), bothCompleted = Boolean(bothJoined && creator.completed_at && guest.completed_at);
  return {
    ok: true,
    room: { code: room.code, caseId: CASE_ID, caseTitle: CASE_TITLE, casePath: CASE_PATH, createdAt: room.created_at, expiresAt: room.expires_at, roomUrl: `${PUBLIC_SITE_ORIGIN}${CASE_PATH}?room=${room.code}` },
    ...
    bothJoined, bothCompleted, results: bothCompleted ? { creator: publicMetrics(creator), guest: publicMetrics(guest) } : null,
  };
};
...
Deno.serve(async (req: Request) => {
  ...
  const action = String(body.action || '').trim(), browserKey = String(body.browserKey || '').trim();
  ...
  if (action === 'create') {
    ...
    for (let attempt = 0; attempt < 6 && !room; attempt += 1) {
      const result = await admin.from('duel_rooms').insert({ code: makeCode(), case_id: CASE_ID, case_title: CASE_TITLE, case_path: CASE_PATH, creator_key_hash: browserKeyHash, expires_at: expiresAt }).select(roomSelect).single();
    ...
    if (playerResult.error) { await admin.from('duel_rooms').delete().eq('id', room.id); return json(503, { error: 'room_create_failed' }, origin); }
  ...
  if (action === 'preview') {
    ...
    return json(200, { ok: true, room: { code, caseTitle: CASE_TITLE }, creatorName: creator?.player_name || 'Сценический следователь', roomFull: Boolean(guest) }, origin);
  }
  if (action === 'join') {
