...
const CASE_ID = 'special:2317';
const CASE_TITLE = 'Последний звонок в 23:17';
const CASE_PATH = '/detektivnye-igry-dlya-dvoih/2317/';
  ...
  if (action === 'create') {
    ...
    for (let attempt = 0; attempt < 6 && !room; attempt += 1) {
      ...
      if (!result.error) room = result.data as Record<string, any>;
        ...
        console.error('coop_2317_room_create_failed', result.error.code, result.error.message);
