...
async function loadOrCreate(roomId:string){
 const rows=await rest(`partner_room_states?select=state,revision,case_id&room_id=eq.${encodeURIComponent(roomId)}&limit=1`),row=Array.isArray(rows)?rows[0]:null;
 if(row){if(row.case_id!==P17_CASE_ID)throw new Error('wrong_state_case');return{state:normalizeP17State(row.state),revision:Number(row.revision)||0};}
 const state=initialP17State();const made=await rest('partner_room_states?select=state,revision,case_id',{method:'POST',headers:{prefer:'return=representation'},body:JSON.stringify({room_id:roomId,case_id:P17_CASE_ID,state,revision:0,entitlement_id:null})});const first=Array.isArray(made)?made[0]:null;if(!first)throw new Error('state_create_failed');return{state:normalizeP17State(first.state),revision:Number(first.revision)||0};
}
