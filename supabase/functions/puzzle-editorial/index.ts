...
const EDITORIAL_KINDS = ['quick', 'expert', 'who_lied_case', 'solo_mini_case'];
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === 'POST') {
    ...
    if (!/^(quick|expert|who_lied_case|solo_mini_case):[A-Za-z0-9_-]+$/.test(id) || !['approved', 'rejected', 'pending'].includes(status)) {
