...
Deno.serve(async (req: Request) => {
  ...
  const amountValue = partnerAmountValue();
  const amount = amountToKopecks(PARTNER_PRICE_RUB);
  if (PARTNER_PRICE_RUB !== 599 || amount !== 59900 || amountValue !== '599.00') {
  ...
  const { data: existing, error: existingError } = await admin
    .from('payment_orders')
    .select('id,token_hash,product_id,status,payment_provider,provider_payment_id,confirmation_url,amount_value,customer_email_hash')
    .eq('client_request_id', requestId)
    .maybeSingle();
  if (existingError) return json(503, { error: 'order_lookup_failed' }, origin);
  if (existing) {
    if (existing.token_hash !== tokenHash || existing.product_id !== PARTNER_PRODUCT_ID) {
    ...
    if (Number(existing.amount_value) !== PARTNER_PRICE_RUB) {
  ...
  const receipt = {
    ...
    Taxation: 'usn_income',
    Items: [{
      Name: PARTNER_RECEIPT_NAME.slice(0, 128),
      Price: amount,
      Quantity: 1,
      Amount: amount,
  ...
  const notificationUrl = `${SUPABASE_URL}/functions/v1/tbank-webhook-partner-v1`;
  ...
  if (!existing) {
    const { error: insertError } = await admin.from('payment_orders').insert({
      ...
      metadata: {
        ...
        charged_price_rub: PARTNER_PRICE_RUB,
        offer_version: OFFER_VERSION,
        offer_accepted_at: acceptedAt,
        privacy_version: PRIVACY_VERSION,
        privacy_acknowledged_at: acceptedAt,
  ...
  try {
    const payment = await tbankRequest('Init', {
      Amount: amount,
      OrderId: orderId,
      Description: PARTNER_DESCRIPTION.slice(0, 140),
      Language: language,
