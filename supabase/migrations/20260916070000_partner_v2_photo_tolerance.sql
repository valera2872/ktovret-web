-- Restore the agreed Partner V2 photo comparison mechanic.
-- New clients submit three physical observations. Two of the three must match
-- the server-side observation profile for that role. Legacy two-field clients
-- retain the previous exact matcher result supplied by the Edge Function.
...
declare
  ...
  v_effective_correct boolean := p_is_correct;
  v_photo_match_count integer := 0;
begin
  ...
  -- The original design uses three stable physical signs and tolerates one
  -- uncertain/mistaken observation. The role-specific truth remains server-side.
  if p_checkpoint_key = 'photo_observation'
     and jsonb_typeof(p_value) = 'object'
     and p_value ? 'door_deformation' then
    if not (p_value ? 'patch' and p_value ? 'scratch' and p_value ? 'door_deformation') then
      return jsonb_build_object('ok', false, 'error', 'invalid_photo_observation');
    end if;

    if (p_value->>'patch') not in ('present', 'absent', 'unsure')
       or (p_value->>'scratch') not in ('present', 'absent', 'unsure')
       or (p_value->>'door_deformation') not in ('present', 'absent', 'unsure') then
      return jsonb_build_object('ok', false, 'error', 'invalid_photo_observation');
    end if;

    if p_role = 'creator' then
      v_photo_match_count :=
        case when p_value->>'patch' = 'absent' then 1 else 0 end +
        case when p_value->>'scratch' = 'absent' then 1 else 0 end +
        case when p_value->>'door_deformation' = 'present' then 1 else 0 end;
    else
      v_photo_match_count :=
        case when p_value->>'patch' = 'present' then 1 else 0 end +
        case when p_value->>'scratch' = 'present' then 1 else 0 end +
        case when p_value->>'door_deformation' = 'absent' then 1 else 0 end;
    end if;

    v_effective_correct := v_photo_match_count >= 2;
  end if;
  ...
  v_both_correct := v_effective_correct and v_other_correct;
  ...
  return jsonb_build_object(
    ...
    'chapter', v_state.chapter,
    'sharedUnlocked', coalesce((v_state.shared_state->>p_shared_key)::boolean, false),
    'photoMatches', case when p_checkpoint_key = 'photo_observation' and p_value ? 'door_deformation' then v_photo_match_count else null end
  );
end;
$$;
